...
module Gitarella
  ...
  class GitarellaCGI
    ...
    def blob_diff
      ...
      @template_params["old"]["sha1"] = params["hp"]
      @template_params["old"]["data"] = @repo.file(@filepath, params["hp"]).encode_entities.split("\n").map(&preprocess)
      @template_params["new"] = Hash.new
      @template_params["new"]["sha1"] = params["hn"]
      @template_params["new"]["data"] = @repo.file(@filepath, params["hn"]).encode_entities.split("\n").map(&preprocess)
    ...
    def static_data(data)
      ...
      response["content-type"] = contentmime
      response.write data
      raise StaticOutput
    end
  end
